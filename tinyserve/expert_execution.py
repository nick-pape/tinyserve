"""Expert execution pipeline with template weight swapping.
...
class ExpertPipeline:
    ...
    def run_layer(
        ...
        """Run one MoE layer: check cache, fetch uncached experts via h2d_transfer, compute.

        Thin alias for execute_layer_experts() with semantic parameter names.
        """
        return self.execute_layer_experts(hidden_states, layer_idx, expert_indices, routing_weights)

    def run_layer_batched(
        self,
        layer_idx: int,
        hidden_states: torch.Tensor,
        expert_indices: torch.Tensor,
        ...
        """Batched variant: groups tokens by expert, loads each expert once.

        O(num_unique_experts) H2D transfers instead of O(seq_len x top_k).
        Thin alias for execute_layer_experts_batched().
        """
        return self.execute_layer_experts_batched(hidden_states, layer_idx, expert_indices, routing_weights)

    def prefetch_for_next_token(
        self,
        layer_idx: int,
        predicted_expert_ids: list[int] | torch.Tensor,
    ) -> None:
        """Initiate h2d_transfer for predicted_expert_ids before the router fires.

        Overlaps transfer with attention compute for the current token.
        Thin alias for schedule_prefetch().
        """
        return self.schedule_prefetch(layer_idx, predicted_expert_ids)
