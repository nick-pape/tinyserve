...
Usage:
    cache = KVCache.preallocate(config, max_context_tokens=4096, device="cuda")
...
class KVCacheOverflow(RuntimeError):
    ...
    Carries `overflow_token_count` — how many additional tokens are required.
    VRAMBudget catches this and frees expert slots to extend the KV cache.
    """

    def __init__(self, overflow_token_count: int):
        self.overflow_token_count = overflow_token_count
        super().__init__(f"KV cache overflow: need {overflow_token_count} more tokens")
...
class KVCache:
    ...
    @classmethod
    def preallocate(cls, config, max_context_tokens=4096,
                    kv_storage_dtype=torch.float8_e4m3fn,
                    device="cuda", storage_device=None) -> "KVCache":
        """Pre-allocate KV buffers for max_context_tokens before inference begins."""
    ...
    # store_layer is the semantic name; update is the HuggingFace Cache interface name (permanent)
    store_layer = update
    ...
    def enable_sliding_window(self, kv_window_tokens: int = 1024, kv_sink_tokens: int = 4) -> None:
        """Enable StreamingLLM-style KV eviction (arxiv 2309.17453).

        Retains the first kv_sink_tokens (attention sinks) and the most recent
        kv_window_tokens. KV memory stays constant regardless of context length.
        """
